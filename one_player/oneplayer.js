 ...
 */
var randomanswer;
var Driddle = document.getElementById("riddling");
...
/* 
    THIS FUNCTION IS IN CHARGE OF THE PLAYER SCORE IT IS ONLY CALLED WHEN A PLAYER GETS THE RIDDLE RIGHT HENCE IT IS EMBEDDED IN THE CHECKIT FUNCTION
    THE CHECKIT FUNCTION WILL BE USED TO CHECK IF THE USER ANSWER IS CORRECT
    */
let currentscore = 0;
var scores = document.getElementById("scor");
var score = () => {
  scores.innerHTML = "score:" + currentscore;
  currentscore += 10;
};

/**
            *THIS FUNCTION IS CALLED WHEN A USER GETS THE RIDDLE RIGHT IT RANDOMLY SELECTS THE ELEMENTS IN ITS ARRAY TO TELL THE USER GOOD OR NICE
            YOU HAVE GOTTEN IT
            */
var y = document.getElementById("cor");
function displaycorrect() {
  ...
  y.textContent = teds[t];
}

/*
 * THIS FUNCTION DELETE THE DISPLAYCORRECT FUNCTION WHEN A NEW RIDDLE IS DISPLAYED
 */
function delit() {
...
function gameoverDisplay() {
  ...
  //We get the box containing the riddles and we change its background for gameover
  var conatainer = document.getElementById("riddle");
  conatainer.style.backgroundImage = "url(blacky.jpg)";

  //This div element is the container that will have the badge image,the nick name,the scores and number of question,we give it id=gameover
  ...
  //CALCULATING THE FINAL SCORES IN PERCENTAGE
  var finalscore = ((currentscore - 10) / (z.length * 10)) * 100;

  //creating scoring:
  var points = document.createElement("p");
  points.id = "scoring";
  points.innerHTML = "Your score is: " + finalscore.toFixed(2) + " %";

  //creating number of questions
  var quiz = document.createElement("p");
  quiz.id = "question";
  quiz.innerHTML = "Number of questions: " + z.length;

  if (finalscore > 90) {
    badge.src = "real_number_1-removebg-preview.png";
    nick.innerHTML = "RIDDLE ROYALTY";
  } else if (finalscore > 70 && finalscore < 90) {
    badge.src = "number two.jpg";
    nick.innerHTML = "BRAIN TEASER TITIAN";
  } else if (finalscore > 50 && finalscore < 70) {
    badge.src = "number three.jpg";
    nick.innerHTML = "RIDDLE RAIDER";
  } else if (finalscore > 30 && finalscore < 50) {
    badge.src = "images__6_-removebg-preview.png";
    nick.innerHTML = "RIDDLE ROOKIE";
  } else {
    badge.src = "images-removebg-preview.png";
    nick.innerHTML = "RIDDLE NOVICE";
  }

  conatainer.appendChild(gameoverbox);
  gameoverbox.appendChild(badge);
  gameoverbox.appendChild(nick);
  gameoverbox.appendChild(points);
  gameoverbox.appendChild(quiz);
}
...
function gameover() {
  ...
  deletetime(); //hoisted:it deletes the timer display
}
...
/******
 * Function TO DELETE TIME DISPLAY FOR GAMEOVER;
 ************/
function deletetime() {
 ...
 */
function automatic() {
  displayRiddle(random()); //displays the riddle
...
window.onload = () => {
  ...
  setInterval(timer, 1000); //counts the time
  ...
  next.onclick = () => {
    displayRiddle(random()); //displays the random number
    clearresponse(); //clear response of the user for next riddle
    delit(); //delete the displaycorrect for next riddle
    resetcheckonce(); //reset the reset the checkit for the next riddle
  };
